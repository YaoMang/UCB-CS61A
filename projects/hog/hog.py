...
def is_always_roll(strategy, goal=GOAL):
    ...
    "*** YOUR CODE HERE ***"
    score1, score2 = 0, 0
    prev, curr = strategy(0, 0), strategy(0, 0)
    while score1 < goal :
        score2 = 0
        while score2 < goal:
            curr = strategy(score1, score2)
            if curr != prev :
                return False
            prev = curr
            score2 += 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while now_rolls <= 10 :
        expect = roll_average(now_rolls, dice)
...
def final_strategy(score, opponent_score, goal=100):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    avg_nextsus = make_averaged(sus_update, 100)
    if score > opponent_score : # Now taking lead
        if sus_update(0, score, opponent_score) > goal :
            return 0
        if avg_nextsus(1, score, opponent_score) > goal :
            return 1
        if avg_nextsus(2, score, opponent_score) > goal :
            return 2
    if sus_update(0, score, opponent_score) > avg_nextsus(6, score, opponent_score) :
        return 0
    else :
        return 6
    # END PROBLEM 12
